...
// Enable COMPONENT_55X & Using Enter Download Mode command for CYW955X devices so skipping minidriver download command
#include <string.h>
#include <stdbool.h>
...
#ifdef COMPONENT_55X
             if (!((p_param->opcode == HCI_VSC_WRITE_RAM) || (p_param->opcode == HCI_VSC_LAUNCH_RAM)))
#else
...
static void cybt_prm_send_next_patch(void)
{
    ...
    if (cybt_prm_cb.format_type == CYBT_PRM_FORMAT_HCD)
    {
        ...
        if ( ( vsc_command == HCI_VSC_LAUNCH_RAM ) && (cybt_prm_cb.tx_patch_length >= cybt_prm_cb.patch_length) )
        {
            cybt_prm_cb.state = CYBT_PRM_ST_LAUNCHING_RAM;
        }
    }
