...
const osThreadAttr_t mqtt_client_task_attributes = {
  .name = "MqttClientTask",
...
int main(void)
{
  ...
  MqttClientTaskHandle = osThreadNew(mqtt_client_task, NULL, &mqtt_client_task_attributes);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, CYBSP_WIFI_WL_REG_ON_Pin|LED_GREEN_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : CYBSP_WIFI_HOST_WAKE_Pin UCPD_FLT_Pin */
  GPIO_InitStruct.Pin = CYBSP_WIFI_HOST_WAKE_Pin|UCPD_FLT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
