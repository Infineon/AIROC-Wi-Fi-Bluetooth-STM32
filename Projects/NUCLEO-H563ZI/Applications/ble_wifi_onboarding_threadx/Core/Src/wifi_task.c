...
void wifi_task(void)
{
    ...
    result = cy_rtos_init_event(&wifi_event);
    ...
    while (true)
    {
        ulNotifiedValue =  NOTIF_EMEEPROM | NOTIF_GATT_DB | NOTIF_SCAN_COMPLETE |
                          NOTIF_DISCONNECT_GATT_DB | NOTIF_DISCONNECT_BTN;
        cy_rtos_waitbits_event(&wifi_event, &ulNotifiedValue, 1, false, CY_RTOS_NEVER_TIMEOUT);
...
void scan_callback(cy_wcm_scan_result_t* result_ptr, void* user_data, cy_wcm_scan_status_t status)
{
    ...
    {
        cy_rtos_setbits_event(&wifi_event, NOTIF_SCAN_COMPLETE, 0);
    }
}
