...
void scan_result_callback ( cy_wcm_scan_result_t *result_ptr, void *user_data, cy_wcm_scan_status_t status )
{
   ...
   if( scan_data != NULL )
   {
      ...
      else
      {
        ...
        {
             if ((strlen((const char *)result_ptr->SSID) != 0) && scan_check_bssid_in_list (result_ptr) == false )
             {
                 scan_data->result_count++;
                 /* Copy BSSID to last bssid which will be used in scan_check_bssid_in_list to avoid
                  * repeating BSSID in scan results
                  */
                 memcpy(&last_bssid, &result_ptr->BSSID[0], sizeof(last_bssid));
                 printf(" %2ld   %-32s     %4d     %3d      %02X:%02X:%02X:%02X:%02X:%02X         %-15s\r\n",
...
void wifi_connect ()
{
   ...
   while (true)
   {
      ...
      {
          printf("Ping failed !! Module %lx Code %lx\r\n", CY_RSLT_GET_MODULE(result), CY_RSLT_GET_CODE(result));
      }
